...
PROCEDURE customFormat:
  ...
  */
END PROCEDURE. /* customFilter */


PROCEDURE customFilter:
  /*
   * Name: customFilter
   * Desc: Give back a value for a field filter
   *
   * Depending on the input parameters you are able to adjust
   * the format of a field. This routine is called after the
   * custom format is fetched from the user settings.
   ...
   * This is handy if - for example - all userid fields in your
   * database have the wrong format. Or if you want to show
   * all date fields with 4 digit years.
   */
  DEFINE INPUT  PARAMETER pcDatabase AS CHARACTER NO-UNDO.
  DEFINE INPUT  PARAMETER pcTable    AS CHARACTER NO-UNDO.
  DEFINE INPUT  PARAMETER pcField    AS CHARACTER NO-UNDO.
  ...
  /* [QAD]
  if pcField matches "*_domain" then pcFilter = global_domain.
  */
END PROCEDURE. /* customFilter */
...
PROCEDURE CustomDump:
  ...
  /* Example code:
   * Ask if user is really, really sure to delete
   */
...
PROCEDURE customQuery:
  ...
  /* Example code: */
  /* Get the most recently used query from query history */
/*  IF pcQuery = "" THEN
