...
PROCEDURE getTables :
/*
 * Fill the ttTable temptable with all tables of all
...
PROCEDURE saveQuery :
/*
 * Save a query to the INI file. Increase the nr of all
 ...
 */
	...
	iFieldOrder = 0.
	REPEAT PRESELECT EACH bField TABLE-SCAN BY bField.iOrder:
		FIND NEXT bField.
		ASSIGN
			iFieldOrder   = iFieldOrder + 1
			bField.iOrder = iFieldOrder.
	...
	iColumnOrder = 0.
	FOR EACH bField TABLE-SCAN BY bField.iOrder:
...
FUNCTION getColor RETURNS INTEGER
	( pcName AS CHARACTER ) :
/*
 * Return the color number for the name given
...
FUNCTION readFile RETURNS LONGCHAR
	(pcFilename AS CHARACTER):
	/* read contents of a file as a longchar. This is used when the
	 * COPY-LOB statement fails as a result of conflicting code-pages.
	 */
